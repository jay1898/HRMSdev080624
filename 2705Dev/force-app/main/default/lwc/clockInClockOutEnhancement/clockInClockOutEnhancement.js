...
import getAttendanceOnTimeArrival from '@salesforce/apex/ClkinClkoutArrivalController.getAttendanceOnTimeArrival';
import getontimeArrival from '@salesforce/apex/ClkinClkoutArrivalController.getontimeArrival';
...
export default class ClockInClockOut extends LightningElement {
    ...
    connectedCallback() {
        ...
        this.ontimeArrival();//onTime Arrived OR Late Arrived
    ...
    handleSmallScreen(){
        ...
        // console.log('small screen in clock in clock out ---------------------------------------------->');
        if(this.windowWidth < 480){
            // console.log('small screen in clock in clock out true ---------------------------------------------->');
        ...
        setTimeout(() => {
            ...
            // console.log('small screen in clock in clock out false---------------------------------------------->');
...
WFHHandleSelection(event) {
    ...
    if (name === 'FromDate' || name === 'ToDate') {
        ...
        if (!isNaN(fromDate.getTime()) && !isNaN(toDate.getTime())) {
            ...
            getNumberOfNonBusinessDays({ startDate: fromDate, endDate: toDate })
                .then(result => {
                    this.totalDays = totalDay - result;
                    //Changes Taken for Work home -1 days
                    this.tempTotalDays = this.totalDays;
                        console.log('Result using apexxxxxx  ---   aaaaaaaaaaaxxxxxxxxxx', this.totalDays);
                        const timeDifference = toDate.getTime() - fromDate.getTime();
                        const daysDifference = timeDifference / (1000 * 3600 * 24);
    ...
    handleSaveClickWFH() {
        ...
        if (!this.wfhRequest.Reason || this.wfhRequest.Reason.trim() === '') {
            ...
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'Reason for Work From Home can not be Empty. ',
                    variant: 'error'
                })
            );
    ...
    handleSaveClickLEAVE(event) {
       ...
       getLeaveRequests({ employeeId: this.empRecordId })
            .then(result => {
                console.log('empRecordId-444444------->',this.empRecordId);
                console.log("getLeaveRequests Result",result);
                if(result.Number_of_Leaves__c != null || result.Number_of_Leaves__c != undefined){
                    this.numberOfAvailableLeaves = result.Number_of_Leaves__c;
                }
    ...
    getAttendanceTotalClockInHoursSinceLogin(givenTime) {
        console.log('this.empRecordId_________', this.empRecordId);
        ...
        getAttendanceOnTimeArrival({ employeeId: this.empRecordId })
            .then(result => {
                ...
                this.attendanceData = result.map(record => ({
                    ...
                    arrivalStatusClass: record.On_Time_Arrival__c ? 'on-time' : 'late'
                }));
                console.log('result this.attendanceData-->',this.attendanceData);
            })
            .catch(error => {
    ...
    ontimeArrival() {
        getontimeArrival({ employeeId: this.empRecordId })
            .then(result => {
                ...
                this.arrivalPercentage.onTime = Math.round((result.OnTimeArrivals * 100) / total) || 0;
                //console.log('this.arrivalPercentage.onTime--->',this.arrivalPercentage.onTime);
                this.arrivalPercentage.late = Math.round((result.lateArrivals * 100) / total) || 0;
                //console.log('this.arrivalPercentage.late---->',this.arrivalPercentage.late);
                console.log('counts', JSON.stringify(this.arrivalPercentage));

                // Check if the sum of percentages exceeds 100%
                const sumOfPercentages = this.arrivalPercentage.onTime + this.arrivalPercentage.late;
                if (sumOfPercentages > 100) {
                    // Adjust the larger percentage downward until the sum is <= 100
                    if (this.arrivalPercentage.onTime > this.arrivalPercentage.late) {
                        this.arrivalPercentage.onTime -= (sumOfPercentages - 100);
                    } else {
                        this.arrivalPercentage.late -= (sumOfPercentages - 100);
                    }
                }
                //console.log('sumOfPercentages-->',sumOfPercentages);
            })
            .catch(error => {
                console.error('Error fetching attendance data:', error);
            });
