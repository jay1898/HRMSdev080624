...
export default class ClockInClockOut extends LightningElement {
    ...
    leaveHandleSelection(event) {
       ...
       // this.getLeaveRequestData();
        if (name === 'FromDate' || name === 'ToDate') {
            ...
            console.log('leaveRequest.FromDate>> ', this.leaveRequest.FromDate);
    ...
    handleClockinoutBtn(event) {
        ...
        if (btnName === 'clockin') {
            ...
            if (btnName === 'clockout') {
                ...
                this.ontimeArrival();//ontimeArrival called on clockout percentage
            }
    ...
    insertClockinRecordsMethod() {
        ...
        insertClockinRecords({ EmpRecordId: this.empRecordId })
            .then(result => {
                ...
                this.loadAttendanceData();//sample
            })
            .catch(error => {
    ...
    ontimeArrival() {
        getontimeArrival({ employeeId: this.empRecordId })
            .then(result => {
                console.log('result', result);
                const arrivals = result.OnTimeArrivals || 0;
                const late = result.lateArrivals || 0;
                
                const total = arrivals + late;
                this.arrivalPercentage.onTime = Math.round((arrivals * 100) / total) || 0;
                //console.log('this.arrivalPercentage.onTime--->',this.arrivalPercentage.onTime);
                this.arrivalPercentage.late = Math.round((late * 100) / total) || 0;
